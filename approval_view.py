...
def format_approval_request(req: ApprovalRequest) -> str:
    if req.request_type == "fs_scope":
        ...
        return f"{req.request_id} | fs_scope | mode={mode} | path={path}"
    if req.request_type == "shell_command":
        command = req.payload.get("command", "")
        return f"{req.request_id} | shell_command | command={command}"
    return f"{req.request_id} | {req.request_type} | payload={req.payload}"
